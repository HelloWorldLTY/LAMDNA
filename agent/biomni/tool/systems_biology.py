...
def create_dna_sequence(question, device='cuda'):
    """
    Call ChatNT to answer a question about a DNA sequence.
    ...
    import sys
    print(sys.path)
    smaller_q = question.lower()
    prompts = [question]
    seq_out = 'nan'
    if 'enhancer' in smaller_q:
        if 'hepg2' in smaller_q or 'k562' in smaller_q or 'sknsh' in smaller_q:
            instruct_len = 30
            from llama_text_pred import CharExpLanguageDataset
            from llama_text_pred import LightningModel
            model = LightningModel.load_from_checkpoint('/home/tl688/pitl688/regLM/tutorials/generation_more_test_value_align_qwen2.53b100epoch_3cl_crosschr_predexp/lightning_logs/version_4/checkpoints/epoch=2-step=4424-v1.ckpt', llm_name = "Qwen/Qwen2.5-3B-Instruct",
        latent_size = 2048, max_token_len=instruct_len)
            model = model.to(device)
            seq_out = model.generate(prompts, max_new_tokens=200, top_k=4, seed=2024)[0]
        else:
            model = LightningModel.load_from_checkpoint('/home/tl688/scratch/generation_more_test_value_align_qwen2.53b100epoch_allhumancellline_predexp/lightning_logs/version_1/checkpoints/epoch=10-step=29900.ckpt', llm_name = "Qwen/Qwen2.5-3B-Instruct",
        latent_size = 2048, max_token_len=instruct_len)
            model = model.to(device)
            seq_out = model.generate(prompts, max_new_tokens=200, top_k=4, seed=2024)[0]
    elif 'promoter' in smaller_q:
        if 'hepg2' in smaller_q or 'k562' in smaller_q or 'sknsh' in smaller_q or 'gm12878' in smaller_q or 'a549' in smaller_q:
            instruct_len = 30
            from llama_text_pred import CharExpLanguageDataset
            from llama_text_pred import LightningModel
            model = LightningModel.load_from_checkpoint('/home/tl688/pitl688/regLM/tutorials/5cl_promoter_train/generation_more_test_value_align_qwen2.53b100epoch_5cl_crosschr_predexp/lightning_logs/version_1/checkpoints/epoch=7-step=10164.ckpt', llm_name = "Qwen/Qwen2.5-3B-Instruct",
        latent_size = 2048, max_token_len=instruct_len)
            model = model.to(device)
            seq_out = model.generate(prompts, max_new_tokens=200, top_k=4, seed=2024)[0]
