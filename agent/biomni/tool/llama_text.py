...
class LightningModel(pl.LightningModule):
    ...
    def embed_data(self,x):
        return self.model.backbone.forward(x[:,2:]).mean(axis=1)
...
#         loss+= F.mse_loss(ins_emb,seq_emb)
    ...
    def configure_optimizers(self):
        optimizer = optim.AdamW(list(self.model.parameters()) + list(self.Llama_encoder.parameters()) + list(self.emb_align.parameters()), lr=float(self.lr))
        return optimizer
