...
class CharExpLanguageDataset(Dataset):
    def __init__(self, seqs, labels, value, seq_len=None, llm_name = "meta-llama/Llama-3.2-3B-Instruct", max_token_len=15):
        ...
        self.value = torch.FloatTensor(value)
    ...
    def __getitem__(self, idx):
        ...
        pred_out = self.value[idx]

        # Encode label
        # label = self.encode_label(label)
        instruct = torch.IntTensor(self.tokenizer(label,padding='max_length', truncation=True, max_length=self.max_token_len, padding_side='left')['input_ids'])
...
def load_pretrained_model(
    ...
    state_dict = torch.load(
        os.path.join(ckpt_dir, "weights.ckpt"), map_location=torch.device("cpu"), weights_only=False
    )
...
#             final_emb = torch.cat((instruct_emb,seq_enc), dim=1)
            ...
            # Return logits or normalized probabilities
            if return_logits:
                return logits, final_emb.mean(axis=1)
            else:
...
#         final_emb = torch.cat((instruct_emb,seq_enc), dim=1)
        ...
        seq_enc = self.model.backbone.forward_multimodal(x[:,:],instruct_emb)
        final_emb = seq_enc
